	...
	<div>
						...
						<el-col :span="5">
							<el-form-item label="业务类型">
								<el-select v-model="form.category" placeholder="请选择" @change="onSubmit">
									<el-option v-for="item in optionsab_type" :key="item.value" :label="item.label"
										:value="item.value">
									</el-option>
								</el-select>
							</el-form-item>
				...
				<el-table ref="multipleTable" class="tablist" max-height="700" @current-change="handleSelectionChange"
					...
					<el-table-column fixed="right" label="操作" width="300">
						<template slot-scope="scope">
							<el-button @click="getBusinChannel(scope.row)" type="primary" size="small">查看渠道</el-button>
				...
				<el-form ref="form" :model="form" label-width="130px">
					...
					<div v-if="categoryseket==7">
						...
						<el-form-item type="textarea" label="评论内容">
							<el-input type="textarea" autosize placeholder="请输入内容" v-model="form.contact">
							</el-input>
						</el-form-item>
					...
					<el-form-item label="启/停">
						<el-select v-model="form.ab_status" placeholder="请选择类型">
							<el-option v-for="item in optionsab_status" :key="item.value" :label="item.label"
								:value="item.value">
							</el-option>
						</el-select>
					</el-form-item>
		...
		<el-dialog title="渠道信息" :visible.sync="editDialogVisible3" width="30%">
			<el-table ref="multipleTable" class="tablist" max-height="700" @current-change="handleSelectionChange"
				...
				<el-table-column prop="channels" label="渠道名称">
				</el-table-column>
				<el-table-column prop="tfollow" label="今日已评论数/重"> </el-table-column>
				<el-table-column prop="yfollow" label="昨日已评论数/重">
				</el-table-column>
		...
		<el-dialog title="渠道信息" :visible.sync="editDialogVisible4" width="30%">
			<el-table ref="multipleTable" class="tablist" max-height="700" @current-change="handleSelectionChange"
				...
				<el-table-column prop="tfollow" label="今日已关注数/重"> </el-table-column>
				<el-table-column prop="yfollow" label="昨日已关注数/重">
				</el-table-column>
			</el-table>
		</el-dialog>
		<div class="block">
	...
	export default {
		...
		methods: {
			...
			getBusinChannel(row) {
				...
				this.axios.get('/public/index.php/getBusinChannel', { params: params })
					.then(res => {
